...
import {
  ...
  checkRule,
  executeActions,
} from '../../apps/rule_engine/ruleEngineCore.js';
...
describe('Rule Engine Core Logic', () => {
  ...
  describe('matchRule (New Structure)', () => {
    ...
    it('should fallback to string match if JSON parse fails', () => {
      const rule = {
  ...
  describe('checkRule (State Clearing)', () => {
    it('should clear throttle state even on mismatch', () => {
      ...
      // 4. Same as step 1 -> SHOULD trigger again (cleared)
      checkRule(rule, {active: true}, callback);
  ...
  describe('executeActions', () => {
    ...
    it('should execute multiple actions immediately if no delay', async () => {
      const rule = {
        then: [
