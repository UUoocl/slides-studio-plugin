...
window.addEventListener('message', async (event) => {
    ...
    if(event.origin === window.location.origin && slidesLoaded == true){  
        ...
        if(data.namespace === "tabulator" && data.message === "slides-data"){
            ...
            notesIframe.contentWindow.postMessage(JSON.stringify({ message:'slides-data-response', slidesData: slidesArray, namespace: 'speakerview'}), window.location.origin);
        }
    } 
    ...
    if (data.namespace === 'reveal' && 
        ['ready'].includes(data.eventName)) {
            ...
            addElementToSlideArray(currentEl);
        }
        ...
        if (data.namespace === 'reveal' && slidesLoaded === false && tableLoaded === false &&
            ...
            addElementToSlideArray(slideEl);
            // console.log("currentEl", slideEl)
            // console.log("currentEl HTML", slideEl.outerHTML)
        }
        
        async function addElementToSlideArray(slideEl){
            //Get slide id        
