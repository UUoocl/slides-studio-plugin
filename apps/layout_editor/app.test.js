/**
 * @fileoverview Unit tests for layout serialization logic.
 */
 ...
 * @param {Object} parent Parent dimensions.
 * @param {Object} controls UI control values.
 * @return {Object} Serialized layout object.
 */
function serializeLayout(name, parent, controls) {
  if (!name) throw new Error('Layout name is required');

  const transform = `scale(${controls.scale}) ` +
      `rotate(${controls.rotate}deg) ` +
      `skew(${controls.skewX}deg, ${controls.skewY}deg)`;
...
/**
 * Formats a value into a CSS string.
...
function formatValue(val) {
  ...
  if (sVal.endsWith('%') || sVal.endsWith('px') || sVal.endsWith('em') ||
      sVal.endsWith('rem') || sVal === 'auto') {
    return sVal;
  }
...
describe('Layout Serialization', () => {
  it('should correctly serialize a complete layout object', () => {
    ...
    expect(result.styles.transform)
        .toBe('scale(1.2) rotate(45deg) skew(10deg, 0deg)');
  });
