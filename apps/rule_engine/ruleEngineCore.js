...
export function exactMatch(target, data) {
  ...
  if (targetKeys.length !== dataKeys.length) {
    return false;
  }
...
export function matchRule(rule, data) {
  ...
  if (matchMode === 'regex' || matchMode === 'wildcard') {
    ...
    const regex = matchMode === 'regex' ?
        new RegExp(payload) : wildcardToRegex(payload);
 ...
 * @param {object} rule The rule configuration.
 * @param {any} data The incoming data.
 * @param {function} triggerCallback Optional callback to execute on trigger.
...
export function checkRule(rule, data, triggerCallback) {
  ...
  if (rule.throttleMode === 'change') {
    ...
    if (now - (rule._lastTriggerTime || 0) < (rule.throttleValue || 0)) {
      return false;
    }
  }
 ...
 * @param {function} publishCallback Callback to publish messages.
 */
