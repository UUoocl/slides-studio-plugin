...
// Get OBS Source Name from URL if available
...
newDiv.innerHTML = `<span>Preset Name</span>
    ...
    <select id="loadSelect" onchange="updateSettingsUI()"><option>Loading...</option></select>
    <button onclick="applyPresetToRender()">Apply</button>
...
window.updateSettingsUI = async () => {
  ...
  try {
    ...
    if (settings) {
        // Apply to the current settings page UI
        setSketchSettings(settings);
        document.getElementById("settingsName").value = presetName;
        
        // Also broadcast real-time settings update so render page tracks selection
...
window.applyPresetToRender = async () => {
    const presetName = document.getElementById("settingsName").value;
    ...
    if (window.stgSocket) {
        ...
        // Update OBS Browser Source URL if we have the source name
        if (obsSourceName) {
            try {
