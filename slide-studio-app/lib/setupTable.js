/**
 * @fileoverview Tabulator initialization for Slides Studio.
 */
...
export function loadTable(options) {
  try {
    if (typeof Tabulator === 'undefined') {
      throw new Error('Tabulator library not found.');
    }
    ...
    /**
     * Matches slide position from scene name.
    ...
    function saveTableToLocalStorage() {
      if (window.slideDeckId && window.table) {
        localStorage.setItem(window.slideDeckId,
            JSON.stringify(window.table.getData()));
      }
    }
